...
def transform_weights(weights, embed, heads):
    ...
    weights['patch_pos/embedding'] = np.concatenate([
        weights['patch_pos/embedding'][:, :1], np.zeros([1, 4, embed], 'float32'),
        weights['patch_pos/embedding'][:, 1:]], axis=1)
    ...
    for key in list(weights.keys()):
        ...
        if any([part in key for part in {'/value/', '/attention_output/', '_expand/', '_squeeze/', '_proj/'}]):
            value = value.T
...
def convert_name(n):
    ...
    n = n.replace('.norm1.weight', '_attn_norm/gamma').replace('.norm1.bias', '_attn_norm/beta')
    n = n.replace('.norm2.weight', '_mlp_norm/gamma').replace('.norm2.bias', '_mlp_norm/beta')
    n = n.replace('.attn.qkv.', '_attn_mhsa/value/').replace('.attn.proj.', '_attn_mhsa/attention_output/')
    n = n.replace('.mlp.fc1.', '_mlp_expand/').replace('.mlp.fc2.', '_mlp_squeeze/')
    n = n.replace('.mlp.w12.', '_mlp_expand/').replace('.mlp.w3.', '_mlp_squeeze/')
    ...
    n = n.replace('norm.weight', 'head_norm/gamma').replace('norm.bias', 'head_norm/beta')
...
if '__main__' == __name__:
    ...
    embed_dim, num_heads = model.get_layer(name='layer_0_attn_mhsa')._query_dense.kernel.shape[:2]
    ...
    for w in model.weights:
        assert w.path in weights_torch, f'Can\'t find weight {w.path} in checkpoint'
    ...
    model.save_weights(path.replace('_linear_head.pth', '_pretrain_linear_head.weights.h5'))
