...
def convert_name(n):
    n = n.replace(
        "Transformer/posembed_input/pos_embedding", "patch_pos/embedding"
    )
    n = n.replace("embedding", "patch/embed").replace(
        "patch_pos/patch/embed", "patch_pos/embedding"
    )
    ...
    n = n.replace("MultiHeadDotProductAttention_1", "attn_mhsa").replace(
        "out", "attention_output"
    )
    n = (
        n.replace("MlpBlock_3", "mlp")
        .replace("Dense_0", "expand")
        .replace("Dense_1", "squeeze")
    )
    ...
    n = n.replace("/embed", "_embed").replace(
        "patch_pos_embedding", "patch_pos/embedding"
    )
...
if "__main__" == __name__:
    parser = argparse.ArgumentParser(
        description="ViT Transformer weight conversion from PyTorch to Keras"
    )
    parser.add_argument(
        "model_type",
        type=str,
